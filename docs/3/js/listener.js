...
HTMLElement.prototype.unlisten = function(name, listener, useCapture=false) {//自身をunlistenする
    if (name) {Listener.remove(this, name, listener, useCapture);}
    else {Listener.removeAll(this)}
}
/*
HTMLElement.prototype.unlistenChildren = function() {//子孫要素をunlistenする
    while (this.firstChild) {
        if (this.firstChild.firstChild) {this.firstChild.firstChild.unlistenChildren()}
        this.firstChild.unlisten();
    }
}
HTMLElement.prototype.unlistenAll = function() {this.unlistenChildren();this.unlisten();}//自身と子孫をunlistenする
*/
// 要素を削除するときunlistenする
...
HTMLElement.prototype.removeChild = function(child) {//指定した子要素nodeを削除する
    if ([...this.children].some(c=>c===child)) {//childが子要素であれば
        child.unlisten();
