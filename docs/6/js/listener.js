...
HTMLElement.prototype.unlisten = function(name, handler, useCapture) {//自身をunlistenする
    Listener.remove(this, name, handler, useCapture);
}
// 要素を削除するときunlistenする
//自身と子孫を削除する（再帰。子孫も削除される。標準APIは再帰されずメモリリークしうるので改善した）
...
//    if (this.parentNode) {const E=this.parentNode.removeChild(this); child.unlisten(); this._removeChild(child);}
//    else {this.unlisten(); this._remove();}
}
//子を削除する（再帰。子孫も削除される。標準APIは再帰されずメモリリークしうるので改善した）
